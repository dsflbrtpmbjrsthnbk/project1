...
class InventoryHubField(models.Model):
    ...
    _description = 'InventoryHub Field Definition & Statistics'
    _order = 'name'
    ...
    stat_top_values = fields.Text(
        string='Top Values',
        readonly=True,
        help='Most frequent values (value: count)',
    )
    ...
    #  Computed summary 
    stat_summary = fields.Char(
        string='Statistics Summary',
        compute='_compute_stat_summary',
        store=False,
    )
        ...
        for rec in self:
            ...
            if t == 'numeric':
                if rec.stat_min or rec.stat_max or rec.stat_avg:
                    rec.stat_summary = (
                        f"Min: {rec.stat_min:.2f} | "
                        f"Max: {rec.stat_max:.2f} | "
                        f"Avg: {rec.stat_avg:.2f}"
                    )
                else:
                ...
                if total:
                    ...
                    rec.stat_summary = (
                        f"True: {rec.stat_true_count} ({pct}%) | "
                        f"False: {rec.stat_false_count}"
                    )
                else:
                    rec.stat_summary = 'No data'
            elif t == 'link':
                rec.stat_summary = (
                    f"Non-null: {rec.stat_non_null_count or 0} / "
                    f"{rec.stat_total_items or 0}"
                )
